...
M.locales = {
  en = {
    ...
    explain_inline_instruction = "Add clear comments to this code explaining what each part does. Do NOT modify the code itself — only add comments above the relevant lines. Return the complete code with comments added, without explanation.",
  ...
  fr = {
    ...
    explain_inline_instruction = "Ajoute des commentaires clairs a ce code pour expliquer ce que fait chaque partie. Ne modifie PAS le code lui-meme — ajoute uniquement des commentaires au-dessus des lignes concernees. Retourne le code complet avec les commentaires ajoutes, sans explication.",
  ...
  ja = {
    ...
    explain_inline_instruction = "このコードに各部分が何をするか説明する明確なコメントを追加してください。コード自体は変更しないでください — コメントのみ追加してください。コメント付きの完全なコードを返してください。説明は不要です。",
